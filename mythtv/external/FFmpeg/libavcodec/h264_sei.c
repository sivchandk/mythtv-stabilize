...
// H264 Closed Captions
static void copy_user_registered_data(H264Context *h, int buf_size)
{
    MpegEncContext * const s = &h->s;
    ...
    if (buf_end - p >= 5 &&
        ...
        if (flags & 0x40) {
            if (buf_end - p < 1)
                return;
            h->s.avctx->dtg_active_format = p[0] & 0x0f;
        }
    } else if (buf_end - p >= 6 &&
...
int ff_h264_decode_sei(H264Context *h){
    ...
    while (get_bits_left(&s->gb) > 16) {
        ...
        case SEI_USER_DATA_REGISTERED_ITU_T_T35:
        	copy_user_registered_data(h, size);
        	get_bits(&s->gb, size*8);//ideally do it with in the function
        	break;
        default:
