...
namespace DTC
{
 ...
 class SERVICE_PUBLIC JobInfoList : public QObject
 {
    Q_OBJECT
    Q_CLASSINFO( "version", "1.0" );

    Q_CLASSINFO( "JobInfo_type", "DTC::JobInfo");
    ...
    Q_PROPERTY( QVariantList JobInfos READ JobInfos DESIGNABLE true )
    ...
    PROPERTYIMP_RO_REF( QVariantList, JobInfos )

  public:

    static void InitializeCustomTypes()
    {
        qRegisterMetaType< JobInfoList  >();
        qRegisterMetaType< JobInfoList* >();

        JobInfo::InitializeCustomTypes();
    }

    JobInfoList(QObject *parent = 0)
        : QObject( parent ),
          m_StartIndex    ( 0 ),
          m_Count         ( 0 ),
          m_TotalAvailable( 0 )
    ...
    JobInfoList( const JobInfoList &src )
    {
        Copy( src );
    }

    void Copy( const JobInfoList &src )
    {
            m_StartIndex    = src.m_StartIndex     ;
            m_Count         = src.m_Count          ;
            m_TotalAvailable= src.m_TotalAvailable ;
            ...
            CopyListContents< JobInfo >( this, m_JobInfos, src.m_JobInfos );
    }

    JobInfo *AddNewJobInfo()
    {
        JobInfo *pObject = new JobInfo( this );
        m_JobInfos.append( QVariant::fromValue<QObject *>( pObject ));
...
Q_DECLARE_METATYPE( DTC::JobInfoList  )
Q_DECLARE_METATYPE( DTC::JobInfoList* )
